# Delta for managed-output

## Purpose

Overview of the curated list, exports, undo, members upload, and tuner lineup numbering. WinUI: `ManagedOutputPage`, `TunerSetupDialog`.
...
- THEN the visible stream URL is shown (truncated is ok; full value remains copyable if C# allows)
...
The system SHALL export a visible-only `.m3u8` and an export-all file that includes backups (matching C# `M3uExporter` / `CurationExporter` comments vs inclusion).
...
- THEN A and B swap numbers (C# `TunerNumbering`)
